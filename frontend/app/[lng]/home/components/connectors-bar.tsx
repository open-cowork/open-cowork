...
import { Chrome, Mail, Calendar, HardDrive, Github, Slack, Database, Command, Layout } from "lucide-react";
        ...
        <>
            ...
            >
                ...
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar mask-gradient-right">
                    {[ConnectorIcons.gmail, ConnectorIcons.calendar, ConnectorIcons.drive, ConnectorIcons.slack, ConnectorIcons.github, ConnectorIcons.notion].map((Icon, i) => (
                        <div key={i} className="flex size-6 shrink-0 items-center justify-center rounded-sm bg-muted text-muted-foreground">
...
function ConnectorsDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    const filteredConnectors = MOCK_CONNECTORS.filter(c => c.type === activeTab || (activeTab === "app" && c.type === "app"));
                        ...
                        <div className="flex-1 overflow-hidden flex flex-col">
                            ...
                            <div className="px-6 py-4 pb-2">
                                <div className="flex items-center justify-between gap-4">
                                    <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ConnectorType)} className="w-auto">
                            ...
                            <div className="flex-1 w-full overflow-y-auto">
                                <div className="p-6 grid grid-cols-2 gap-4 pb-20">
                                    {filteredConnectors.map((connector) => {
                                        const isAppAndNotGithub = connector.type === 'app' && connector.id !== 'github';
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <div className="font-medium truncate">{connector.title}</div>
                                                        {isAppAndNotGithub && (
                                                            <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground whitespace-nowrap">敬请期待</span>
                                                        )}
                                                    </div>
                                                    <div className="text-sm text-muted-foreground line-clamp-2">{connector.description}</div>
                                                </div>
                                            </div>
                                        );
                                    })}
...
function ConnectorDetail({ connector, onBack }: { connector: Connector; onBack: () => void }) {
    return (
            ...
            <div className="border-t border-[#333] p-6">
                <div className="grid grid-cols-1 gap-4 text-sm">
                    <div className="flex justify-between items-center py-2 border-b border-[#333]/50">
                        ...
                        <span className="capitalize">{connector.type === 'app' ? '应用' : connector.type === 'mcp' ? 'MCP' : 'Skill'}</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-[#333]/50">
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-[#333]/50">
                        ...
                        <span className="font-mono text-xs text-muted-foreground flex items-center gap-2">
                            f2a3...b9c1 <span className="cursor-pointer hover:text-foreground">📄</span>
                        </span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-[#333]/50">
                        <span className="text-muted-foreground">网站</span>
                        <a href={connector.website} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-blue-300 flex items-center gap-1">
                            Link ↗
                        </a>
                    </div>
                    <div className="flex justify-between items-center py-2">
                        <span className="text-muted-foreground">隐私政策</span>
                        <a href={connector.privacyPolicy} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-blue-300 flex items-center gap-1">
                            Link ↗
                        </a>
                    </div>
                </div>
                <div className="mt-8 text-center">
                    <Button variant="link" className="text-muted-foreground h-auto p-0 text-xs">提供反馈</Button>
                </div>
            </div>
        </div>
    )
