...
import type { ChatMessage, ChatSession, MessageRole } from "@/lib/types";
...
需要我详细展开某个部分吗？`;
  ...
  const addMessage = React.useCallback(
    async (content: string, role: MessageRole) => {
      ...
      if (role === "user") {
        try {
          const response = await chatApi.createSession(content);
          // Save user prompt to localStorage for execution page
          localStorage.setItem(
            `session_prompt_${response.session_id}`,
            content,
          );
          // Redirect to execution page
          window.location.href = `/chat/${response.session_id}`;
        } catch (error) {
